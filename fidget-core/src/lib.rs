...
//! [`fidget_core::context::Context`](crate::context::Context).  A context serves as
...
//! a simple Rust-native scripting language, in the [`fidget_core::rhai`
//! namespace](crate::rhai).  These bindings allow shapes to be constructed from
//! a script, adding flexibility:
//!
//! ```
//! # use fidget_core::context::Context;
//! let t = fidget_core::rhai::engine().eval("x + y")?;
//! let mut ctx = Context::new();
//! let sum = ctx.import(&t);
//! # Ok::<(), fidget_core::Error>(())
//! ```
...
//!
//! - [`fidget_core::vm::VmFunction`](crate::vm::VmFunction) evaluates a list of
//!   opcodes using an interpreter.  This is slower, but can run in more
//!   situations (e.g. in WebAssembly).
//! - [`fidget_core::jit::JitFunction`](crate::jit::JitFunction) performs fast
...
//! implemented in the [`fidget_core::render` module](render).
//!
...
//!
//! This is documented in the [`fidget_core::mesh`](mesh) module.
//!
//! # WebAssembly notes
