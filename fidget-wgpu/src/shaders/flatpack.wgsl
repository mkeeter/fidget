...
@group(1) @binding(2) var<storage, read> dispatch: VoxelDispatch;

/// Scratch buffer used to pack tiles (one item per Z position)
...
fn flatpack_main(
    ...
    let buffer_offset = tile_z_to_offset[z];
    let count = atomicAdd(&tile_count[z], 1u);
