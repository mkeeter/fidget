...
mod test {
    ...
    mod gpu {
        ...
        fn step_by_step_x_(ctx: &Context) {
            ...
            let shape = ctx.shape(&VmShape::from(Tree::x())).unwrap();
        ...
        fn step_by_step_y_(ctx: &Context) {
            ...
            let buffers =
                ctx.buffers(size, GpuSpec::Custom(MAX_TILES_PER_DISPATCH));
            let shape = ctx.shape(&VmShape::from(Tree::y())).unwrap();
            let settings = RenderConfig::default();
        ...
        fn repeated_xy_(ctx: &Context) {
            ...
            let shape_x = ctx.shape(&VmShape::from(Tree::x())).unwrap();
            let shape_y = ctx.shape(&VmShape::from(Tree::y())).unwrap();
            let settings = RenderConfig::default();
