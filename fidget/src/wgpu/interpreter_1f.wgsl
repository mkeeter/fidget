...
fn run_tape(start: u32, inputs: vec4f) -> f32 {
    var i: u32 = start;
    ...
    while (true) {
        ...
        switch op[0] {
            ...
            case OP_COPY_IMM:    { reg[op[1]] = read_imm_f32(&i); }
            ...
            case OP_ADD_REG_IMM:  { reg[op[1]] = reg[op[2]] + read_imm_f32(&i); }
            case OP_MUL_REG_IMM:  { reg[op[1]] = reg[op[2]] * read_imm_f32(&i); }
            case OP_DIV_REG_IMM:  { reg[op[1]] = reg[op[2]] / read_imm_f32(&i); }
            case OP_SUB_REG_IMM:  { reg[op[1]] = reg[op[2]] - read_imm_f32(&i); }
            case OP_MOD_REG_IMM:  { reg[op[1]] = mod_f32(reg[op[2]], read_imm_f32(&i)); }
            case OP_ATAN_REG_IMM: { reg[op[1]] = atan2(reg[op[2]], read_imm_f32(&i)); }
            case OP_COMPARE_REG_IMM:  { reg[op[1]] = compare_f32(reg[op[2]], read_imm_f32(&i)); }

            case OP_DIV_IMM_REG:      { reg[op[1]] = read_imm_f32(&i) / reg[op[2]]; }
            case OP_SUB_IMM_REG:      { reg[op[1]] = read_imm_f32(&i) - reg[op[2]]; }
            case OP_MOD_IMM_REG:      { reg[op[1]] = mod_f32(read_imm_f32(&i), reg[op[2]]); }
            case OP_ATAN_IMM_REG:     { reg[op[1]] = atan2(read_imm_f32(&i), reg[op[2]]); }
            case OP_COMPARE_IMM_REG:  { reg[op[1]] = compare_f32(read_imm_f32(&i), reg[op[2]]); }

            case OP_MIN_REG_IMM:  { reg[op[1]] = min(reg[op[2]], read_imm_f32(&i)); }
            case OP_MAX_REG_IMM:  { reg[op[1]] = max(reg[op[2]], read_imm_f32(&i)); }
            case OP_AND_REG_IMM:  { reg[op[1]] = and_f32(reg[op[2]], read_imm_f32(&i)); }
            case OP_OR_REG_IMM:   { reg[op[1]] = or_f32(reg[op[2]], read_imm_f32(&i)); }
