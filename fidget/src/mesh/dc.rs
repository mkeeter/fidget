...
pub trait DcCore {
    ...
    fn dc_cell(&mut self, cell: CellIndex) -> bool {
        if let Cell::Branch { index, .. } =
            self.octree().cells[cell.index].into()
        {
            debug_assert_eq!(index % 8, 0);
            for i in Corner::iter() {
                self.push(Task::Cell(self.octree().child(cell, i)));
            }
    ...
    fn dc_edge<T: Frame + FrameToTask>(
        ...
        if cs.iter().all(|v| self.octree().is_leaf(*v)) {
            ...
            for j in 0..4 {
                self.triangle(vs[j], vs[(j + winding) % 4], i);
            }
            false
        } else {
            let (t, u, v) = T::frame();
